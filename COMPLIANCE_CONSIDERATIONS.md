...
|---|---|---|
| Access controls | Unique user identification, authentication, and authorization before PHI access | Not applicable -- this app has no login and accepts no patient-identifying input by design. If real PHI were ever introduced, this would need real authentication. |
...
| Encryption at rest | PHI stored at rest should be encrypted | Operational data and minimized audit events are in PostgreSQL. Encryption at rest depends on deployment storage configuration and is not provided by this repository alone. |
